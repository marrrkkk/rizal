...
function App() {
  ...
  useEffect(() => {
    // Initialize session from SessionManager
    const initializeSession = () => {
      ...
      if (sessionManager.isAdminSessionValid()) {
        ...
        setUsername(adminSession.user.username);
        setIsAdmin(true);
      ...
      if (sessionManager.isUserSessionValid()) {
    ...
    const handleStorageChange = (e) => {
      if (e.key && (e.key.includes('session') || e.key.includes('admin') || e.key.includes('user'))) {
        initializeSession();
      }
    };
    ...
    // Check for session changes more frequently (for same-tab updates after login)
    ...
    if (process.env.NODE_ENV === "development") {
      ...
      window.getSessionInfo = () => sessionManager.getCurrentSessionInfo();
      // console.log("🧪 Developer tools available:");
      // console.log("  - window.getProgressData() - View current progress data");
      // console.log("  - window.refreshProgress() - Refresh progress from database");
      // console.log("  - window.getSessionInfo() - View current session info");
    }
  ...
  const handleLogout = () => {
    ...
    // Clear appropriate session based on current session type
  ...
  // Helper function to validate user session for protected routes
  const validateUserSession = () => {
    ...
    return sessionType === 'user' && !isAdmin;
  };

  // Helper function to validate admin session for admin routes
  const validateAdminSession = () => {
    ...
    return sessionType === 'admin' && isAdmin;
  };

  // Protected Route Component for User Routes
  const ProtectedUserRoute = ({ children }) => {
    return validateUserSession() ? children : <Navigate to="/login" />;
  };

  // Protected Route Component for Admin Routes
  const ProtectedAdminRoute = ({ children }) => {
    return validateAdminSession() ? children : <Navigate to="/admin/login" />;
  };
      ...
      <BrowserRouter>
        ...
        <Routes>
          ...
          <Route
            path="/login"
            element={
              (sessionType !== 'user' && !token) ? (
                <Login
                  setToken={(token) => {
                    // Handle user login through SessionManager
                    try {
                      ...
                      if (tokenParts.length === 3) {
                        ...
                        sessionManager.setUserSession(token, userInfo);
                        setToken(token);
          ...
          <Route
            ...
            element={
              (sessionType !== 'user' && !token) ? (
                <Register
                  setToken={(token) => {
                    // Handle user registration through SessionManager
                    try {
                      const tokenParts = token.split(".");
                      if (tokenParts.length === 3) {
                        const payload = JSON.parse(atob(tokenParts[1]));
                      ...
                      console.error("Error processing registration token:", error);
                    }
                  }}
                />
              ) : (
