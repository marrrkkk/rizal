...
export default function Chapter1({ username, onLogout }) {
  ...
  // All 5 levels are now unlocked for demo
  const [unlockedLevels] = useState([1, 2, 3, 4, 5])
      ...
      <header className="bg-white/80 backdrop-blur-sm shadow-sm sticky top-0 z-10">
        <div className="w-full px-6 py-4 flex justify-between items-center">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-800">Chapter 1: Childhood in Calamba</h1>
              <p className="text-sm text-gray-600">Choose a level to continue learning</p>
            </div>
          </div>
          <button
            onClick={onLogout}
      ...
      <main className="w-full px-6 py-8">
        ...
        <div className="text-center mb-12">
          <div className="inline-block p-6 bg-white/60 backdrop-blur-sm rounded-3xl shadow-lg mb-6">
            ...
            <h2 className="text-4xl font-bold text-gray-800 mb-2">Childhood in Calamba</h2>
            <p className="text-lg text-gray-600 max-w-2xl mx-auto">
              Discover the early years of Jose Rizal in his hometown of Calamba, Laguna
            </p>
          </div>
        </div>
        ...
        <div className="mb-8">
          ...
          <p className="text-center text-sm text-gray-600 mt-2">
            {unlockedLevels.length} of {levels.length} levels available
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6 max-w-6xl mx-auto">
          {levels.map((level, index) => {
            ...
            const isCompleted = unlockedLevels.includes(level.id) && level.id < Math.max(...unlockedLevels)

            return (
              <div
                key={level.id}
                className={`group relative overflow-hidden rounded-2xl shadow-lg transition-all duration-300 transform cursor-pointer ${
                  isUnlocked ? "bg-white hover:shadow-2xl hover:-translate-y-2" : "bg-gray-100 opacity-60"
                }`}
              ...
              >
                ...
                <div className="absolute top-4 left-4 z-10">
                  ...
                  >
                    <span className={`text-lg font-bold ${isUnlocked ? "text-gray-800" : "text-gray-500"}`}>
                      {level.id}
                    </span>
                  </div>
                </div>
                ...
                <div className="relative p-6 pt-16">
                  ...
                  <h3
                    className={`text-lg font-bold mb-2 transition-colors ${
                      isUnlocked ? "text-gray-800 group-hover:text-gray-900" : "text-gray-500"
                    }`}
                  >
                    {level.title}
                  </h3>
        ...
        {unlockedLevels.length === levels.length && (
          <div className="mt-12 bg-gradient-to-r from-green-400 to-emerald-500 text-white rounded-3xl p-8 shadow-xl max-w-4xl mx-auto text-center">
            ...
            <p className="text-xl text-green-100 mb-4">
              Congratulations! You've learned all about Jose Rizal's childhood in Calamba!
            </p>
            <div className="bg-white/20 rounded-2xl p-4 inline-block">
              <span className="text-2xl font-bold">🏆 Childhood Expert Badge Earned!</span>
            </div>
          </div>
        )}
        ...
        <div className="mt-16 bg-white/60 backdrop-blur-sm rounded-3xl p-8 shadow-lg max-w-4xl mx-auto">
          <h3 className="text-2xl font-bold text-center text-gray-800 mb-6">About This Chapter</h3>
              ...
              <div className="space-y-3">
                <div className="flex items-center space-x-3">
                  <div className="w-8 h-8 bg-blue-100 rounded-full flex items-center justify-center">
                    <span className="text-blue-600 font-bold text-sm">1861</span>
                  </div>
                  <span className="text-gray-600">Year Jose Rizal was born</span>
                </div>
                <div className="flex items-center space-x-3">
                  <div className="w-8 h-8 bg-green-100 rounded-full flex items-center justify-center">
                    <span className="text-green-600 font-bold text-sm">3</span>
                  </div>
                  <span className="text-gray-600">Age when he learned to read</span>
                </div>
                <div className="flex items-center space-x-3">
                  <div className="w-8 h-8 bg-purple-100 rounded-full flex items-center justify-center">
                    <span className="text-purple-600 font-bold text-sm">👩</span>
                  </div>
                  <span className="text-gray-600">His mother was his first teacher</span>
                </div>
                <div className="flex items-center space-x-3">
                  <div className="w-8 h-8 bg-orange-100 rounded-full flex items-center justify-center">
                    <span className="text-orange-600 font-bold text-sm">📚</span>
                  </div>
                  <span className="text-gray-600">Loved reading from a very young age</span>
                </div>
              </div>
            </div>
          </div>
