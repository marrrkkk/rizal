...
export default function EuropeanJourneyGame({ onComplete }) {
  ...
  const events = [
    {
      year: 1882,
      event: "Departed for Spain",
      description: "Left Philippines to study in Spain",
      location: "Manila to Barcelona",
    },
    {
      year: 1883,
      event: "Moved to Madrid",
      description: "Studied medicine and philosophy",
      location: "Madrid, Spain",
    },
    {
      year: 1885,
      event: "Earned Medical Degree",
      description: "Graduated as a physician",
      location: "Madrid, Spain",
    },
    {
      year: 1886,
      event: "Studied in Paris",
      description: "Specialized in ophthalmology",
      location: "Paris, France",
    },
    {
      year: 1887,
      event: "Published Noli Me Tangere",
      description: "First novel published in Berlin",
  ...
  const startNewRound = () => {
    ...
    setShuffledOptions(
      [...otherYears, currentEvent.year].sort(() => 0.5 - Math.random())
    );
    setFeedback("");
    setIsCorrect(null);
    ...
    return (
      <div className="max-w-2xl mx-auto p-6 bg-white rounded-lg shadow-md text-center">
        ...
        <p className="text-lg mb-6">
          Score: {score} out of {events.length}
        </p>
        <button
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 bg-white rounded-lg shadow-md">
      ...
      <div className="flex justify-between items-center text-sm text-gray-500">
        <div>
          Question {currentEventIndex + 1} of {events.length}
        </div>
        <div>Score: {score}</div>
      </div>
    </div>
